 ...
 *       the maximum capacity is limited by the amo unt of free memory on the
...
public class DoubleArraySequence implements Cloneable {
	...
	public void addAll(DoubleArraySequence addend) {
		...
		this.ensureCapacity(temp.manyItems + this.manyItems);
	...
	public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
		...
		for (int i = 0; i < d1.data.length; i++) {
			newData[inc] = d1.data[i];
			inc++;
		}
		for (int j = 0; j < d2.data.length; j++) {
			newData[inc] = d2.data[j];
			inc++;
		}
		DoubleArraySequence temp = new DoubleArraySequence(newData.length);
		
		temp.currentIndex = inc;
		temp.manyItems = d1.manyItems + d2.manyItems;
		temp.data = newData;
		return temp;
	...
	public boolean isCurrent() { // see if sequence has a specified current element
		...
		return (manyItems > currentIndex) || (data.length == 0);
	}
